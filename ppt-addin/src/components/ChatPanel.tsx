...
import { readDocumentContext } from '../services/contextReader';
...
const STORAGE_KEY = 'iwrite-ppt-addin-chat-history';
...
function loadHistory(): ChatMessage[] {
  ...
  return [{ role: 'ai', content: '你好！请描述你的 PowerPoint 演示文稿生成需求。' }];
}
...
export default function ChatPanel({ onOutlineGenerated }: ChatPanelProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      // 读取当前演示文稿上下文
      const docContext = await readDocumentContext();

      const response = await apiClient.post('/chat', {
        message: trimmed,
        context: { documentContent: docContext },
        format: 'ppt',
      });
        ...
        <div className={styles.inputRow}>
          <div className={styles.textareaWrap}>
            <Textarea
              ...
              placeholder="例如：帮我生成一份 Q3 季度销售汇报幻灯片..."
              resize="none"
              rows={1}
