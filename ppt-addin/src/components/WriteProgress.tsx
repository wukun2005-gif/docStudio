...
import { writeToPresentation } from "../services/pptWriteService";
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        const resp = await fetch('/api/generation/ppt', {
          method: 'POST',
        ...
        const poll = async () => {
          ...
          try {
            ...
            if (data.status === 'done') {
              ...
              // 使用成熟方案写入 PowerPoint：
              // 服务端 PptxGenJS 生成带原生图表的 PPTX → 客户端 insertSlidesFromBase64 插入
              if (typeof (window as any).PowerPoint !== 'undefined') {
                try {
                  await writeToPresentation({ slides: [], runId });
                } catch (e) {
                  console.error('[WriteProgress] writeToPresentation 失败:', e);
                  if (cancelledRef.current) return;
                  setError(e instanceof Error ? e.message : '演示文稿写入失败，请检查 Office API 可用性后重试');
                  return;
                }
              }
              ...
              // 使用 data.title 和 outline 构建基本信息（PptxGenJS 生成的 PPTX 结构更丰富，这里提供基本数据）
              const sections: GenerationSection[] = outlineRef.current.map(item => ({
                title: item.title,
  ...
  return (
    <div className={styles.root}>
      <div className={styles.header}>
        <Text className={styles.title}>正在生成演示文稿</Text>
      </div>
      ...
      <Text>{completedCount} / {total} 张幻灯片</Text>
