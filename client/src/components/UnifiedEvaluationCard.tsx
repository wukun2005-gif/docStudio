...
/** 无评估数据时的占位展示数据（中英双语，避免英文模式下露出中文示例） */
const DEMO_ZH: EvalMetrics = {
...
const DEMO_EN: EvalMetrics = {
  groundedness: { score: 0.6497142857142858 },
  relevance: { score: 1, irrelevantSentences: [] },
  completeness: { score: 0.9333333333333333, missingPoints: [] },
  conflicts: { hasConflicts: true, conflictRate: 0.25, items: [
    { topic: "Q3 team size: target vs. actual headcount", claims: [{ text: "Current team: 18; Q3 target: 25", source: "Product-Roadmap-Q3-2026.pptx" }, { text: "Headcount: 18-19", source: "docs/PRD.md" }], severity: "low" },
    { topic: "Beta release date", claims: [{ text: "July: Beta release", source: "Product-Roadmap-Q3-2026.pptx" }, { text: "Sprint 3 did not complete the Beta prerequisites", source: "10-Chen-Qiang-Weekly-Summary.eml" }], severity: "medium" },
    { topic: "Q3 technical debt items cleared", claims: [{ text: "14 technical debt items cleared in Q3", source: "Q3-Architecture-Evolution-Report.docx" }, { text: "14 items cleared in Q3 (categorised)", source: "Q3-Architecture-Evolution-Report.docx" }], severity: "low" },
    { topic: "Payment API refactor go-live date", claims: [{ text: "Go-live: 10 September 2026", source: "Q3-Architecture-Evolution-Report.docx" }, { text: "Sprint 3 only produced a draft design", source: "10-Chen-Qiang-Weekly-Summary.eml" }], severity: "medium" },
    { topic: "Total Q3 commits", claims: [{ text: "Q3 total commits: 2,553", source: "14-Chen-Qiang-Q3-Code-Productivity-Weekly.eml" }, { text: "Q3 total: 1,096 commits", source: "Q3-GitHub-Activity-Report.docx" }], severity: "high" },
  ] },
};
...
export default function UnifiedEvaluationCard({ evaluationMetrics, trustScore }: Props) {
  ...
  const m: EvalMetrics = hasAllMetrics(evaluationMetrics) ? evaluationMetrics : (locale === "en" ? DEMO_EN : DEMO_ZH);

  const radar = [
    { lab: t("evaluation.grounded"), val: m.groundedness?.score ?? 0 },
    { lab: t("evaluation.relevant"), val: m.relevance?.score ?? 0 },
    { lab: t("evaluation.complete"), val: m.completeness?.score ?? 0 },
    { lab: t("evaluation.noConflict"), val: m.conflicts ? 1 - m.conflicts.conflictRate : 1 },
  ];

  const issues = useMemo(() => {
    const list: Array<{ t: string; d: string; s: string }> = [];
    if (trustScore != null && trustScore < 0.8) list.push({ t: "unsupported", d: t("evaluation.unsupportedDesc", { score: Math.round(trustScore * 100) }), s: t("evaluation.unsupportedSuggestion") });
    // 展开每一条无关句子，让用户看到具体内容并可在文档中定位
    if (m.relevance?.irrelevantSentences?.length) for (const sentence of m.relevance.irrelevantSentences) list.push({ t: "irrelevant", d: `${t("evaluation.irrelevantPrefix")}${sentence.length > 120 ? sentence.substring(0, 120) + "…" : sentence}`, s: t("evaluation.irrelevantSuggestion") });
    if (m.completeness?.missingPoints?.length) for (const p of m.completeness.missingPoints) list.push({ t: "uncovered", d: `${t("evaluation.uncoveredPrefix")}${p}`, s: t("evaluation.uncoveredSuggestion") });
    if (m.conflicts?.items?.length) for (const c of m.conflicts.items) {
      const claimsDesc = c.claims.map(cl => {
        const src = cl.source ? (locale === "en" ? ` (${cl.source})` : `（${cl.source}）`) : "";
        return `"${cl.text}"${src}`;
      }).join("  vs  ");
      list.push({ t: "blocked", d: `${t("evaluation.blockedPrefix")}${c.topic}${t("evaluation.blockedSuffix")}\n${claimsDesc}`, s: t("evaluation.blockedSuggestion") });
    }
    if (list.length === 0 && trustScore != null) list.push({ t: "unsupported", d: t("evaluation.allGood"), s: t("evaluation.allGoodSuggestion") });
    return list;
  }, [m, trustScore, t, locale]);

  const META: Record<string, { i: string; l: string }> = {
    unsupported: { i: "🔴", l: t("evaluation.issueUnsupported") },
    irrelevant: { i: "🟠", l: t("evaluation.issueIrrelevant") },
    uncovered: { i: "🟡", l: t("evaluation.issueUncovered") },
    blocked: { i: "🟢", l: t("evaluation.issueBlocked") },
  };
  ...
  if (collapsed) return (
    <div className="border rounded-lg bg-white overflow-hidden" id="demo-eval-card">
      <button id="demo-eval-expand" onClick={() => setCollapsed(false)} className="w-full px-4 py-2 flex items-center justify-between text-xs text-gray-500 hover:bg-gray-50">
        <span>📊 {t("evaluation.title")}</span><span>▼ {t("evaluation.expand")}</span>
      </button>
    </div>
  );
  ...
  return (
    <div className="border rounded-lg bg-white overflow-hidden" id="demo-eval-card">
      <div className="flex border-b bg-gray-50 items-center">
        <button className={`flex-1 px-3 py-2 text-xs font-medium ${tab === "overview" ? "bg-white text-indigo-700 border-b-2 border-indigo-500" : "text-gray-500"}`} onClick={() => setTab("overview")}>📊 {t("evaluation.overview")}</button>
        <button id="demo-eval-tab-issues" className={`flex-1 px-3 py-2 text-xs font-medium ${tab === "issues" ? "bg-white text-indigo-700 border-b-2 border-indigo-500" : "text-gray-500"}`} onClick={() => setTab("issues")}>🔍 {t("evaluation.issues", { count: issues.length })}</button>
        <button id="demo-eval-collapse" onClick={() => setCollapsed(true)} className="px-2 py-2 text-xs text-gray-400 hover:text-gray-600" title={t("evaluation.collapse")}>▲</button>
      </div>
      {tab === "overview" && (
      ...
      {tab === "issues" && (
        <div className="p-3 max-h-[360px] overflow-y-auto space-y-3">
          {Array.from(grouped.entries()).map(([t, items]) => {
            ...
            return (
              <div key={t}>
                <div className="flex items-center gap-1.5 mb-1.5"><span className="text-xs">{meta.i}</span><span className="text-xs font-medium text-gray-700">{meta.l}{locale === "en" ? ` (${items.length})` : `（${items.length}）`}</span></div>
                {items.map((item, i) => (
