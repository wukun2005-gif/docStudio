...
function parseChartSpecs(content: string, chartSpecsRaw?: string[]): ExcelWritePayload["sheets"][number]["charts"] {
  ...
  if (chartSpecsRaw && chartSpecsRaw.length > 0) {
    for (const raw of chartSpecsRaw) {
      try {
        ...
        const items: ChartSpec[] = Array.isArray(parsed) ? parsed : [parsed];
  ...
  // Fallback: 从 HTML content 中的 ```chart 代码块提取
  if (charts.length === 0) {
 ...
 */
export function toExcelPayload(result: GenerateDocResult): ExcelWritePayload {
  logger.info(`[ExcelPayloadBuilder] 开始转换: ${result.sections.length} 个 section → Excel payload`);
