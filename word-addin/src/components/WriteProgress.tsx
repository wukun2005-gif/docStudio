...
import { writeToDocument } from "../services/wordWriteService";
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        const resp = await fetch('/api/generation/word', {
          method: 'POST',
        ...
        const poll = async () => {
          ...
          try {
            ...
            if (data.status === 'done') {
              ...
              // 写入 Word 文档
              if (data.wordPayload && typeof Word !== 'undefined') {
                try {
                  await writeToDocument(data.wordPayload);
                } catch (e) {
                  console.error('[WriteProgress] writeToDocument 失败:', e);
                }
              }
              ...
              const sections: GenerationSection[] = (data.wordPayload?.sections ?? [])
                .map((s: { title: string; paragraphs?: Array<{ text?: string }> }) => ({
                  title: s.title,
  ...
  return (
    <div className={styles.root}>
      <div className={styles.header}>
        <Text className={styles.title}>正在生成 Word 文档</Text>
      </div>
