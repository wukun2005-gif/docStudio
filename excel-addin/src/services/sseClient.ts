...
export async function consumeSSE(
  ...
  let retryCount = 0;
  const MAX_RETRIES = 3;
  const WATCHDOG_MS = 300_000; // 5 分钟无事件则判定连接断开（LLM 每个 section 可能需要 60+ 秒）

  // watchdog：如果超过 WATCHDOG_MS 没有收到任何事件，主动中断
  let watchdogTimer: ReturnType<typeof setTimeout> | null = null;
  const resetWatchdog = () => {
    if (watchdogTimer) clearTimeout(watchdogTimer);
    watchdogTimer = setTimeout(() => {
      console.error('[SSE] watchdog 超时，60 秒未收到事件，主动中断连接');
      onEvent({ type: 'error', message: '连接超时：60 秒未收到服务器响应，可能网络中断。请刷新后重试。' });
      controller.abort();
    }, WATCHDOG_MS);
  };

  // 外部 signal + 内部 watchdog controller 组合
  const controller = new AbortController();
  const onExternalAbort = () => controller.abort();
  signal?.addEventListener('abort', onExternalAbort);

  try {
    resetWatchdog(); // 启动 watchdog
    ...
    await fetchEventSource(url, {
      ...
      onmessage(ev) {
        resetWatchdog(); // 每收到一个事件，重置 watchdog
        try {
